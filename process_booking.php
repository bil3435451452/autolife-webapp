...
try {
    ...
    $carBrand = trim($input['car-brand']);
    $carModel = trim($input['car-model']);
    $service = trim($input['service-select']);
    $date = trim($input['booking-date']);
    $additionalInfo = trim($input['additional-info'] ?? '');
...
function sendEmailNotification($data) {
    ...
    $subject = "Новая заявка с сайта AutoLife Detail";
